    ...
    <div style={{ background: theme.bg, color: theme.fg, minHeight: '100vh' }}>
      ...
      <section style={{ padding: '120px 0 80px' }}>
        <div style={{ maxWidth: 1320, margin: '0 auto', padding: '0 40px' }}>
          ...
          <h1 style={{ fontFamily: FONTS.display, fontStyle: 'italic', fontWeight: 500, fontSize: 'clamp(72px, 9vw, 132px)', letterSpacing: -2.4, lineHeight: 0.95, margin: '32px 0 56px', color: theme.fg, maxWidth: 1100 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 80, alignItems: 'start' }}>
            <div style={{ fontFamily: FONTS.display, fontStyle: 'italic', fontSize: 28, lineHeight: 1.4, color: theme.fg, fontWeight: 500, letterSpacing: -0.3 }}>
              Divine Dialogue is a small studio that builds a single product: a private, reverent companion for the questions you don't bring to the dinner table.
            </div>
      ...
      <section style={{ padding: '100px 0', background: theme.surface, borderTop: `1px solid ${theme.line}`, borderBottom: `1px solid ${theme.line}` }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 40px', textAlign: 'center' }}>
          <div style={{ color: accent, marginBottom: 28, display: 'flex', justifyContent: 'center' }}><Glyph size={36} color={accent} /></div>
          <blockquote style={{ fontFamily: FONTS.display, fontStyle: 'italic', fontSize: 'clamp(40px, 5vw, 64px)', letterSpacing: -1, lineHeight: 1.2, color: theme.fg, fontWeight: 500, margin: 0 }}>
            "An algorithm cannot give you a soul. But it can sit beside you while you find yours."
          </blockquote>
          <div style={{ fontFamily: FONTS.mono, fontSize: 11, letterSpacing: 2.4, color: theme.muted, marginTop: 28 }}>FROM THE FOUNDING NOTE · MARCH 2024</div>
        </div>
      </section>
      ...
      <section style={{ padding: '160px 0' }}>
        <div style={{ maxWidth: 1320, margin: '0 auto', padding: '0 40px' }}>
          ...
          <h2 style={{ fontFamily: FONTS.display, fontStyle: 'italic', fontWeight: 500, fontSize: 'clamp(48px, 6vw, 84px)', letterSpacing: -1.6, lineHeight: 1, margin: '32px 0 80px', color: theme.fg }}>
            Three years of<br/>quiet conversation.
          </h2>
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {STORY.map((s, i) => (
              <Reveal key={i} delay={i * 80}>
                <div style={{ display: 'grid', gridTemplateColumns: '160px 1fr', gap: 60, padding: '40px 0', borderTop: `1px solid ${theme.line}`, alignItems: 'baseline' }}>
                  <div style={{ fontFamily: FONTS.display, fontStyle: 'italic', fontSize: 56, fontWeight: 500, color: accent, letterSpacing: -1 }}>{s.year}</div>
                  <div style={{ fontFamily: FONTS.display, fontSize: 'clamp(24px, 2.6vw, 34px)', letterSpacing: -0.4, lineHeight: 1.3, color: theme.fg, maxWidth: 820, fontWeight: 400 }}>{s.text}</div>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      <section style={{ padding: '160px 0', background: theme.surface, borderTop: `1px solid ${theme.line}` }}>
        <div style={{ maxWidth: 1320, margin: '0 auto', padding: '0 40px' }}>
          <SectionLabel theme={theme} accent={accent}>WHAT WE WON'T DO</SectionLabel>
          <h2 style={{ fontFamily: FONTS.display, fontStyle: 'italic', fontWeight: 500, fontSize: 'clamp(48px, 6vw, 84px)', letterSpacing: -1.6, lineHeight: 1, margin: '32px 0 64px', color: theme.fg, maxWidth: 900 }}>
            Our boundaries,<br/>plainly stated.
          </h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 24 }}>
            {VALUES.map(([h, b], i) => (
              <Reveal key={i} delay={i * 70}>
                <div style={{ background: theme.bg, border: `1px solid ${theme.line}`, borderRadius: 20, padding: '36px 32px' }}>
                  <div style={{ fontFamily: FONTS.display, fontStyle: 'italic', fontSize: 30, fontWeight: 500, color: theme.fg, letterSpacing: -0.4, marginBottom: 12 }}>{h}</div>
