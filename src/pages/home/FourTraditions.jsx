...
export default function FourTraditions() {
  ...
  const gridCols = isMobile || isTablet ? 'repeat(2, 1fr)' : 'repeat(4, 1fr)';
  const cardMinHeight = isMobile ? 280 : 360;
  const cardPadding = isMobile ? '24px 20px' : '36px 28px';
  const watermarkFontSize = isMobile ? 64 : 92;
  const bookTitleFontSize = isMobile ? 22 : 28;
  const headerFlexDirection = isMobile ? 'column' : 'row';
  const headerAlignItems = isMobile ? 'flex-start' : 'flex-end';
  const headerJustifyContent = isMobile ? 'flex-start' : 'space-between';
  const descMaxWidth = isMobile ? '100%' : 380;

  return (
    <section style={{ background: theme.surface, padding: sectionPadding, borderTop: `1px solid ${theme.line}` }}>
      <div style={{ maxWidth: 1320, margin: '0 auto', padding: containerPadding }}>
        <Reveal>
          <div style={{ display: 'flex', flexDirection: headerFlexDirection, justifyContent: headerJustifyContent, alignItems: headerAlignItems, marginBottom: 64, gap: 40, flexWrap: 'wrap' }}>
            <div>
              <SectionLabel theme={theme} accent={accent}>FOUR TRADITIONS</SectionLabel>
              <h2 style={{ fontFamily: FONTS.display, fontStyle: 'italic', fontWeight: 500, fontSize: headingFontSize, letterSpacing: -1.6, lineHeight: 1, margin: '32px 0 0', color: theme.fg, maxWidth: 800 }}>
                One conversation,<br/>many doorways.
              </h2>
            </div>
            <div style={{ fontFamily: FONTS.ui, fontSize: 16, lineHeight: 1.55, color: theme.muted, maxWidth: descMaxWidth }}>
            ...
            return (
              <Reveal key={it.id} delay={i * 80}>
                <button onClick={() => setReligionId(it.id)} style={{ cursor: 'pointer', border: 'none', textAlign: 'left', width: '100%', padding: 0, background: 'transparent' }}>
                  <div style={{ background: theme.bg, border: `1px solid ${active ? r.accent : theme.line}`, borderRadius: 24, padding: cardPadding, minHeight: cardMinHeight, display: 'flex', flexDirection: 'column', transition: 'transform 400ms ease, border-color 400ms ease', transform: active ? 'translateY(-6px)' : 'none', position: 'relative', overflow: 'hidden' }}>
                    <div style={{ position: 'absolute', right: -10, bottom: -20, fontFamily: it.font, direction: it.dir, fontSize: watermarkFontSize, color: r.accent, opacity: 0.08, lineHeight: 1, fontWeight: 500, pointerEvents: 'none' }}>{it.script}</div>
                    ...
                    <div style={{ flex: 1 }} />
                    <div style={{ fontFamily: FONTS.display, fontStyle: 'italic', fontWeight: 500, fontSize: bookTitleFontSize, letterSpacing: -0.4, lineHeight: 1.05, color: theme.fg, marginBottom: 8, position: 'relative' }}>{r.book}</div>
